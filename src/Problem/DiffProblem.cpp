...
const int DiffProblem::m_argsPows[ARGS_NUM] = { 2, 3 };
...
IDiffProblem* DiffProblem::clone() const {
    ...
    if (clone->setParamsDomain(m_paramsDomain) != RC::SUCCESS ||
		clone->setParams(m_params) != RC::SUCCESS ||
		clone->setArgsDomain(m_argsDomain) != RC::SUCCESS ||
		clone->setArgs(m_args) != RC::SUCCESS) {
...
RC DiffProblem::evalGradientByArgs(IVector const* const& args, IVector* const& val) const {
	...
	double firstCoord = 2 * paramsData[A] * pow(argsData[X], m_argsPows[X] - 1) +
